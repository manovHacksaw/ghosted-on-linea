...
contract Ghosted {
    enum ApplicationStatus { Submitted, Viewed, Shortlisted, Rejected, InterviewSchlinealed, Hired }
    ...
    event LineaTokensAwarded(address indexed user, uint256 amount, string reason);
    ...
    function onboardUser(
        ...
        _awardLineaTokens(msg.sender, 100, "Initial onboarding reward");
    ...
    function _awardLineaTokens(
        address _user,
        uint256 _amount,
        string memory _reason
    ) internal {
        userProfiles[_user].lineaTokenBalance += _amount;
        emit LineaTokensAwarded(_user, _amount, _reason);
    }
    ...
    function submitApplication(string memory _jobId, string memory _metadataURI) external returns (bytes32) {
        ...
        _awardLineaTokens(msg.sender, 10, "Application submission reward");
    ...
    function recordView(bytes32 _appId) external {
        ...
        _awardLineaTokens(msg.sender, 5, "Application review reward");
        _awardLineaTokens(applications[_appId].applicant, 5, "Application viewed reward");
    ...
    function updateStatus(bytes32 _appId, ApplicationStatus _newStatus) external {
        ...
        if (_newStatus == ApplicationStatus.InterviewSchlinealed) {
            _awardLineaTokens(applications[_appId].applicant, 20, "Interview schlinealed reward");
        } else if (_newStatus == ApplicationStatus.Hired) {
            _awardLineaTokens(applications[_appId].applicant, 100, "Hiring reward");
            _awardLineaTokens(msg.sender, 50, "Successful hiring reward");
        }
